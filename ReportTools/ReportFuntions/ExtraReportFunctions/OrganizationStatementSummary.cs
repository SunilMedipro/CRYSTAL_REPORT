...
{
    internal class OrganizationStatementSummaryReport
    {
        public static ReportDocument CrystalRerportDocument; 

        public static string ReportFileName = @"Reports\ExtraReports\OrganizationStatementSummaryRpt.rpt";


        public static void OrganizationStatementSummaryReportCall(string baseUrl, ReportDocument cryRpt,
            string initdate, string finaldate, string ADATE, string BDATE, string from_date,
            string company1, string address1, string userid, int paymode, string orgid)
        {
            ...
            //  initdate+ "&finaldate="+finaldate+"&userid="+userid+"&paymode="+paymode).Result;
                   ...
                   var response = client.GetAsync(baseUrl + "/api/ReportSection/OrganizationStatementSummarySelect?init=" + initdate.ToString() + "&final=" + finaldate.ToString()
              + "&orgid=" + orgid.ToString()).Result;

            var responseString = response.Content.ReadAsStringAsync().Result;

            if (response.IsSuccessStatusCode == true)
            {
                var test = JsonConvert.DeserializeObject<List<OrganizationStatementSummary_dt>>(responseString);
        ...
        public class OrganizationStatementSummary_dt
        {
