...
{
    internal class ServicePanelReport
    {
        public static ReportDocument CrystalRerportDocument; 

        public static string ReportFileName = @"Reports\ExtraReports\Catalogue\ServicePanelRpt.rpt";


        public static void ServicePanelReportCall(string baseUrl, ReportDocument cryRpt,
            ...
            string company1, string address1, string userid, int paymode)
        {
            ...
            //  initdate+ "&finaldate="+finaldate+"&userid="+userid+"&paymode="+paymode).Result;
                    ...
                    var response = client.GetAsync(baseUrl + "/api/ServicePanel/Report").Result;

            var responseString = response.Content.ReadAsStringAsync().Result;

            if (response.IsSuccessStatusCode == true)
            {
                var test = JsonConvert.DeserializeObject<List<ServicePanelReport_dt>>(responseString);
        ...
        public class ServicePanelReport_dt
        {
