...
{
    internal class ConsultantClassSetupReport
    {
        public static ReportDocument CrystalRerportDocument; 

        public static string ReportFileName = @"Reports\ExtraReports\Catalogue\ConsultantClassSetupRtp.rpt";


        public static void ConsultantClassSetupReportCall(string baseUrl, ReportDocument cryRpt,
            ...
            string company1, string address1, string userid, int paymode)
        {
            ...
            //  initdate+ "&finaldate="+finaldate+"&userid="+userid+"&paymode="+paymode).Result;
                    ...
                    var response = client.GetAsync(baseUrl + "/api/RefererSetup/ConsultantClassSetup/ConsultantNameGet").Result;

            var responseString = response.Content.ReadAsStringAsync().Result;

            if (response.IsSuccessStatusCode == true)
            {
                var test = JsonConvert.DeserializeObject<List<ConsultantClassSetupReport_dt>>(responseString);
        ...
        public class ConsultantClassSetupReport_dt
        {
