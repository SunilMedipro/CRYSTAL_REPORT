...
{
    internal class HospitalWardSetupReport
    {
        public static ReportDocument CrystalRerportDocument; 

        public static string ReportFileName = @"Reports\ExtraReports\Catalogue\HospitalWardSetupRpt.rpt";


        public static void HospitalWardSetupReportCall(string baseUrl, ReportDocument cryRpt,
            ...
            string company1, string address1, string userid, int paymode)
        {
            ...
            //  initdate+ "&finaldate="+finaldate+"&userid="+userid+"&paymode="+paymode).Result;
                    ...
                    var response = client.GetAsync(baseUrl + "/api/Ward/select" ).Result;

            var responseString = response.Content.ReadAsStringAsync().Result;

            if (response.IsSuccessStatusCode == true)
            {
                var test = JsonConvert.DeserializeObject<List<HospitalWardSetupReport_dt>>(responseString);
        ...
        public class HospitalWardSetupReport_dt
        {
            public int warddno { get; set; }
