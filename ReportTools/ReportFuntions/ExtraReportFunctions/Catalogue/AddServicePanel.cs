...
{
    internal class AddServicePanelReport
    {
        public static ReportDocument CrystalRerportDocument; 

        public static string ReportFileName = @"Reports\ExtraReports\Catalogue\AddServicePanelRpt.rpt";


        public static void AddServicePanelReportCall(string baseUrl, ReportDocument cryRpt,
            ...
            string company1, string address1, string userid, int paymode)
        {
            ...
            //  initdate+ "&finaldate="+finaldate+"&userid="+userid+"&paymode="+paymode).Result;
                    ...
                    var response = client.GetAsync(baseUrl + "/api/ServPanel/Report").Result;

            var responseString = response.Content.ReadAsStringAsync().Result;

            if (response.IsSuccessStatusCode == true)
            {
                var test = JsonConvert.DeserializeObject<List<AddServicePanel_dt>>(responseString);
        ...
        public class AddServicePanel_dt
        {
