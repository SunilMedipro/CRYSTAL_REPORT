...
{
    internal class OrganizationDepartmentSummaryReport
    {
        public static ReportDocument CrystalRerportDocument; 

        public static string ReportFileName = @"Reports\ExtraReports\OrganizationDepartmentSummaryRpt.rpt";


        public static void OrganizationDepartmentSummaryReportCall(string baseUrl, ReportDocument cryRpt,
            string initdate, string finaldate, string ADATE, string BDATE, string from_date,
            string company1, string address1, string userid, int paymode, string orgid, string dptid)
        {
            ...
            //  initdate+ "&finaldate="+finaldate+"&userid="+userid+"&paymode="+paymode).Result;
                   ...
                   var response = client.GetAsync(baseUrl + "/api/ReportSection/OrganizationDepartmentSummarySelect?init=" + initdate.ToString() + "&final=" + finaldate.ToString()
              + "&orgid=" + orgid.ToString() + "&dptid=" + dptid.ToString()).Result;

            var responseString = response.Content.ReadAsStringAsync().Result;

            if (response.IsSuccessStatusCode == true)
            {
                var test = JsonConvert.DeserializeObject<List<OrganizationDepartmentSummary_dt>>(responseString);
        ...
        public class OrganizationDepartmentSummary_dt
        {
